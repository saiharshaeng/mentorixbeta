...
(function () {
  ...
  async function loadBankFiles() {
    ...
    // DATA INTEGRITY NOTE: Banks marked quarantined=true have corrupt answer distributions
    // (dominant answer >70%). Generated by broken OCR pipeline. DO NOT serve to students.
    // CANONICAL SOURCE: fixed/ directory contains normalized, validated versions.
    const banks = [
      // fixed/ canonical banks — single source of truth for practice questions
    ...
    for (const bank of banks) {
      ...
      try {
        ...
        const normalized = validRaw.map((q, i) => {
          ...
          // Handle correct answer — NEVER default to [0]/answer 'a' for unverified questions
          let ans = [];
        ...
        // Silent fail — bank unavailable, not a runtime error
      }
    }
